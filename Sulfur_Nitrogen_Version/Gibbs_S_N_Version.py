...
# ------------------------------------
# Reaction 0: Na2O (silicate) + SiO2 (silicate) <-> Na2SiO3 (silicate)
G0=-(log_to_ln*(-1.33+13870.0/TK))*R*TK  #Magma code line 809
G0=-G0  #our reaction is reverse of that on line 809 of Magma code
GRT0=G0/(R*TK)

# ------------------------------------
# Reaction 1: 0.5 Si (metal) + FeO (silicate) <-> Fe (metal) + 0.5 SiO2 (silicate)
G1=0.5*GmetalSi+GmeltFeO-GmetalFe-0.5*GmeltSiO2
GRT1=G1/(R*TK)

# ------------------------------------
# Reaction 2: MgO (silicate) + SiO2 (silicate) <-> MgSiO3 (silicate)
#G3=-(log_to_ln*(0.42+2329.0/TK))*R*TK
G2=GmeltSiO2+GmeltMgO-GmeltMgSiO3
GRT2=G2/(R*TK)

# ------------------------------------
# Reaction 3: 0.5 Si (metal) + O (metal) <-> 0.5 SiO2 (silicate) # check sign
...
# ------------------------------------
# Reaction 4: H2 (silicate) <-> 2 H (metal)
G4=GmeltH2-2.0*GmetalH
GRT4=G4/(R*TK)

# ------------------------------------
# Reaction 5: FeO (silicate) + SiO2 (silicate) <-> FeSiO3 (silicate)
...
# ------------------------------------
# Reaction 6: SiO2 (silicate) + 2 H2 (silicate) <-> 2 H2O (silicate) + Si (metal)
G6=2.0*GmeltH2+GmeltSiO2-GmetalSi-2.0*GmeltH2O
GRT6=G6/(R*TK)

# ------------------------------------
# Reaction 7: CO2 (gas) <-> CO (gas) + 0.5 O2 (gas)
G7=GgasCO2-GgasCO-0.5*GgasO2
GRT7=G7/(R*TK)

# ------------------------------------
# Reaction 8: 2 H2 (gas) + CO (gas) <-> CH4 (gas) + 0.5 O2 (gas)
G8=2.0*GgasH2+GgasCO-GgasCH4-0.5*GgasO2
GRT8=G8/(R*TK)

# ------------------------------------
# Reaction 9: H2O (gas) <-> 0.5 O2 (gas) + H2 (gas)
G9=GgasH2O-0.5*GgasO2-GgasH2
GRT9=G9/(R*TK)

# ------------------------------------
# Reaction 10: 0.5 O2 (gas) + Fe (gas) <-> FeO (silicate)
G10=0.5*GgasO2+GgasFe-GmeltFeO
GRT10=G10/(R*TK)

# ------------------------------------
# Reaction 11: 0.5 O2 (gas) + Mg (gas) <-> MgO (silicate)
G11=0.5*GgasO2+GgasMg-GmeltMgO
GRT11=G11/(R*TK)

# ------------------------------------
# Reaction 12: 0.5 O2 (gas) + SiO (gas) <-> SiO2 (silicate)
G12=0.5*GgasO2+GgasSiO-GmeltSiO2
GRT12=G12/(R*TK)

# ------------------------------------
# Reaction 13: 0.5 O2 (gas) + 2 Na (gas) <-> Na2O (silicate)
G13=0.5*GgasO2+2.0*GgasNa-GmeltNa2O
GRT13=G13/(R*TK)

# ------------------------------------
# Reaction 14: H2 (silicate) <-> H2 (gas)
G14=GmeltH2-GgasH2  #Self consistent with above
GRT14=G14/(R*TK)

# ------------------------------------
# Reaction 15: H2O (silicate) <-> H2O (gas)
G15=GmeltH2O-GgasH2O  #Self consistent with above
GRT15=G15/(R*TK)

# ------------------------------------
# Reaction 16: CO (silicate) <-> CO (gas)
...
# ------------------------------------
# Reaction 17: CO2 (silicate) <-> CO2 (gas)
G17=5200.0-TK*(-119.77)
GRT17=G17/(R*TK)

# ------------------------------------
# Reaction 18: SiH4 (gas) + 0.5 O2 (gas) <-> SiO (gas) + 2 H2 (gas)
...
# ------------------------------------
# Reaction 22: 2 FeSO4 (silicate) <-> 2 FeO (silicate) + 2 SO2 (gas) + O2 (gas)
G22 = 2.0*GmeltFeSO4 - 2.0*GmeltFeO - 2.0*GgasSO2 - GgasO2
GRT22=G22/(R*TK)

# ------------------------------------
# Reaction 23: SO2 (gas) + H2 (gas) <-> H2S (gas) + O2 (gas)
G23=GgasSO2 + GgasH2 - GgasH2S - GgasO2
GRT23=G23/(R*TK)

# ------------------------------------
# Reaction 24: 3 H2O (silicate) + FeS (silicate) <-> 3 H2 (silicate) + FeO (silicate) + SO2 (gas)
...
# ------------------------------------
# Reaction 26: 2 NH3 (gas) <-> 3 H2 (gas) + N2 (gas)
G26 = 2.0*GgasNH3 - 3.0*GgasH2 - GgasN2
GRT26 = G26/(R*TK)

# ------------------------------------
# Reaction 27: NH3 (gas) + CH4 (gas) <-> HCN (gas) + 3 H2 (gas)
